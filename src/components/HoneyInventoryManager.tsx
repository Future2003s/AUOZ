...
import { DefectiveReportModal } from '@/features/inventory/components/DefectiveReportModal';
import { DefectiveView } from '@/features/inventory/components/DefectiveView';
...
export default function HoneyInventoryManager() {
  ...
  const [activeView, setActiveView] = useState('all'); // 'all', 'low', 'premium', 'history', 'defective'
  const [isDefectiveModalOpen, setIsDefectiveModalOpen] = useState(false);
  const [defectiveTargetItem, setDefectiveTargetItem] = useState<InventoryItem | null>(null);
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 font-sans overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <InventorySidebar
          showSidebar={showSidebar}
          activeView={activeView as 'all' | 'low' | 'premium' | 'history' | 'defective'}
          onChangeView={(view) => {
            setActiveView(view);
                        ...
                        {formattedHistory.length > 0 ? (
                          formattedHistory.map((item) => (
                            <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                              ...
                              <td className="px-3 sm:px-6 py-3 sm:py-4">
                                <span className={`inline-flex items-center px-2 sm:px-2.5 py-1 rounded-full text-xs font-medium border ${item.type === 'import'
                          ...
                          inventory.map((item) => {
                            ...
                            return (
                              <tr key={item.id} className={`group transition-all border-b border-slate-100/50 dark:border-slate-800/60 ${selectedItems.includes(item.id) ? 'bg-blue-50/60 dark:bg-blue-900/30' : 'bg-white/40 dark:bg-slate-900/40 hover:bg-amber-50/40 dark:hover:bg-amber-900/20'}`}>
                                ...
                                <td className="px-4 sm:px-6 py-4">
                                  <span className={`inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-semibold border shadow-sm ${isLarge
                                ...
                                <td className="px-4 sm:px-6 py-4 text-right">
                                  <div className="flex items-center justify-end space-x-1.5 opacity-100 sm:opacity-70 sm:group-hover:opacity-100 transition-opacity">
                                    ...
                                    <button
                                      onClick={() => { setDefectiveTargetItem(item); setIsDefectiveModalOpen(true); }}
                                      ...
                                      title="Báo hàng lỗi"
                                      aria-label={`Báo hàng lỗi cho ${item.name}`}
                                    >
                  ...
                  {inventory.length > 0 ? (
                    inventory.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className={`bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm border rounded-2xl p-4 shadow-md transition-all ${selectedItems.includes(item.id)
                        ...
                        >
                          ...
                          <div className="grid grid-cols-2 gap-3 mb-3">
                            <div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-3 border border-slate-200/60 dark:border-slate-700/80">
                              ...
                              <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold border ${isLarge
                                ? 'bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-900/30 dark:text-purple-300 dark:border-purple-800'
                                : 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800'
                                }`}>
                          ...
                          <div className="flex items-center justify-between pt-3 border-t border-slate-200/60 dark:border-slate-700/80">
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <button
                                onClick={() => { setDefectiveTargetItem(item); setIsDefectiveModalOpen(true); }}
                                className="p-2.5 text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 rounded-xl transition-transform active:scale-95 shadow-sm"
                                title="Báo hàng lỗi"
                                aria-label={`Báo hàng lỗi cho ${item.name}`}
                              >
                                <AlertTriangle className="w-4 h-4" />
                ...
                <div className="col-span-2">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${Number(currentItem.netWeight) === 165
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${Number(currentItem.netWeight) === 435
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={`w-full h-full text-center px-1 rounded-xl border focus:outline-none focus:ring-2 ${[165, 435].includes(Number(currentItem.netWeight))
      ...
      {isStockModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in overflow-y-auto">
          <div className="bg-white dark:bg-slate-950 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-md my-auto overflow-hidden border border-slate-200 dark:border-slate-800">
            <div className={`px-5 sm:px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center backdrop-blur-sm ${stockAction.type === 'import'
              ? 'bg-green-50/60 dark:bg-green-900/20'
              : 'bg-rose-50/60 dark:bg-rose-900/20'
              }`}>
              <h3 className={`text-lg sm:text-xl font-bold ${stockAction.type === 'import'
            ...
            <form onSubmit={handleSaveStock} className="p-5 sm:p-6 bg-white dark:bg-slate-950">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6 bg-slate-50 dark:bg-slate-900 p-1.5 rounded-2xl border border-slate-200/60 dark:border-slate-700/80">
                <button
                  ...
                  className={`flex items-center justify-center space-x-2 py-3 rounded-xl text-sm font-semibold transition-all ${stockAction.type === 'import'
                ...
                <button
                  ...
                  className={`flex items-center justify-center space-x-2 py-3 rounded-xl text-sm font-semibold transition-all ${stockAction.type === 'export'
              ...
              <div className="space-y-5 mb-6">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-4 border-2 rounded-2xl text-center text-3xl font-bold outline-none ring-2 transition-all shadow-md ${stockAction.type === 'import'
              ...
              <div className="flex justify-end space-x-3">
                ...
                <button
                  type="submit"
                  className={`px-5 py-2.5 text-sm font-semibold text-white rounded-2xl shadow-md disabled:opacity-50 disabled:cursor-not-allowed transition-transform flex items-center space-x-2 hover:-translate-y-0.5 active:translate-y-0 ${stockAction.type === 'import'
      ...
      <DefectiveReportModal
        ...
        onClose={() => { setIsDefectiveModalOpen(false); setDefectiveTargetItem(null); }}
        item={defectiveTargetItem}
      />
