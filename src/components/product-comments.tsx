...
const formatTimeAgo = (value: string, localeObj: any) => {
    try {
        return formatDistanceToNow(new Date(value), { addSuffix: true, locale: localeObj });
    } catch {
...
export default function ProductCommentsSection({ productId }: { productId: string }) {
    ...
    const t = useCallback((key: string, variables?: Record<string, string | number>) => {
        let text = tFull(`products.comments.${key}`);
        if (variables) {
    ...
    const parseResponse = (payload: CommentApiResponse | null, fallbackMessage = t("error_generic")) => {
    ...
    const fetchComments = useCallback(
        async (pageToLoad = 1) => {
            ...
            try {
                ...
                const parsed = parseResponse(payload, t("error_load_failed"));
    ...
    const handleDelete = useCallback(
        async (commentId: string) => {
            if (!isAuthenticated) {
                toast.error(t("toast_login_required_delete"));
                return;
            }
            ...
            try {
                ...
                const text = await res.text();
                const payload = text ? (JSON.parse(text) as CommentApiResponse) : null;
                parseResponse(payload, t("error_delete_failed"));
    ...
    const handleSubmit = useCallback(async () => {
        if (!content.trim()) {
            toast.error(t("toast_content_required"));
            return;
        }
        setSubmitting(true);
        try {
        ...
        <div className="mt-12 bg-gray-50 py-8 px-4 sm:px-6 lg:px-8 font-sans rounded-3xl border border-gray-200 shadow-sm">
            <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="p-6 border-b border-gray-100 bg-white">
                    <h2 className="text-2xl font-bold text-gray-900 mb-6">{t("section_title")}</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                        ...
                        <div className="md:col-span-4 flex flex-col items-center justify-center p-6 bg-yellow-50 rounded-xl border border-yellow-100">
                            {totalComments > 0 ? (
                                <>
                                    ...
                                    <div className="text-gray-600 text-sm font-medium">{t("feedback_count", { count: totalComments })}</div>
                                </>
                            ) : (
                                <>
                        ...
                        <div className="md:col-span-8 flex flex-col justify-center gap-2">
                            <div className="flex items-center gap-3">
                                <div className="text-sm font-medium text-gray-600 w-12 text-right">{t("stats_total")}</div>
                                ...
                                <div className="text-xs text-gray-400 w-14 text-right">{t("stats_comments_count", { count: totalComments })}</div>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="text-sm font-medium text-gray-600 w-12 text-right">{t("stats_people")}</div>
                                <div className="flex-1 h-2.5 bg-gray-100 rounded-full overflow-hidden">
                                    <div
                                ...
                                <div className="text-xs text-gray-400 w-14 text-right">{t("stats_people_count", { count: distinctCommenters })}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-4 bg-gray-50 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4 sticky top-0 z-10 backdrop-blur-sm bg-opacity-95">
                    ...
                    >
                        ...
                        {isWritingReview ? t("btn_close") : t("btn_write")}
                    </button>
                </div>

                {isWritingReview && (
                    <div className="p-6 bg-white border-b border-gray-100 animate-fadeIn">
                        <h3 className="text-lg font-semibold text-gray-800 mb-4">{t("form_title")}</h3>
                        <div className="mb-4">
                            <label className="block text-sm font-medium text-gray-700 mb-2">{t("form_label")}</label>
                            <div className="flex items-center gap-2 mb-3">
                                <span className="text-sm text-gray-600">{t("form_rating")}</span>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="text-sm text-gray-500">{content.length}/1000 {t("form_chars")}</div>
                                    ...
                                    <div className="flex-1">
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2">
                                            <div>
                                                <h4 className="font-bold text-gray-900 text-base">
                                                    {comment.user?.name || t("guest_user")}
                                                </h4>
                                                <div className="flex items-center gap-2 mt-1 text-xs text-gray-500">
                                                    <span>{formatTimeAgo(comment.createdAt, dateLocale)}</span>
                                                    {comment.isEdited && <span className="italic text-gray-400">{t("edited_mark")}</span>}
                                                    {comment.rating ? (
                                        ...
                                        <div className="flex items-center gap-6 mt-2 text-sm text-gray-500">
                                            <button className="flex items-center gap-1.5 hover:text-indigo-600 transition-colors">
                                                <ThumbsUp size={16} />
                                                <span>{t("action_helpful")} ({comment.likeCount || 0})</span>
                                            </button>
                                            <button className="flex items-center gap-1.5 hover:text-indigo-600 transition-colors">
                                                <MessageSquare size={16} />
                                                <span>{t("action_reply")} ({comment.replyCount || 0})</span>
                                            </button>
                                            ...
                                            <div className="mt-4 space-y-3 rounded-xl border border-gray-100 bg-white/70 p-4">
                                                {comment.replies.map((reply) => (
                                                    <div key={reply.id} className="flex gap-3">
                                                        <Avatar className="h-8 w-8">
                                                            {reply.user?.avatar && (
                                                                <AvatarImage src={reply.user.avatar} alt={reply.user?.name || t("guest_user")} />
                                                            )}
                                                        ...
                                                        <div className="flex-1">
                                                            <div className="flex items-center gap-2 text-sm">
                                                                <span className="font-semibold text-gray-900">
                                                                    {reply.user?.name || t("guest_user")}
                                                                </span>
                                                                <span className="text-xs text-gray-500">{formatTimeAgo(reply.createdAt, dateLocale)}</span>
                                                            </div>
                                                ...
                                                {comment.replyCount > (comment.replies?.length || 0) && (
                                                    <p className="text-xs text-gray-500">
                                                        +{t("more_replies", { count: comment.replyCount - (comment.replies?.length || 0) })}
                                                    </p>
                                                )}
                                            </div>
                                        )}
